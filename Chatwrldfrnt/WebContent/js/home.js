var chatworld=angular.module("chatworld",['ngRoute','ngCookies']);
...
chatworld.run( function ($rootScope, $location,$cookieStore, $http) {
...
chatworld.service("ChatService", function($q, $timeout) {
    ...
    service.SOCKET_URL = "/Chatworld/chat";
...
chatworld.controller("chatController",function($scope,$http,ChatService,$rootScope)
		{
		...
		chatworld.controller("JobsController",function($scope,$http)
			...
			$scope.addJ=function()
			{
				...
				chatworld.controller('logoutController',function($scope,$rootScope,$http,$cookieStore)		
						{
					$http.post("http://localhost:8081/Chatworld/logout/"+$rootScope.uname);
							 ...
							 $scope.newForum={};
								...
								$scope.answer=function(forum)
								{
									...
									answer:$scope.answerforum.answer
									 ...
									 $http.post('http://localhost:8081/Chatworld/answerForum',ans);
									$http.get("http://localhost:8081/Chatworld/viewForum")
								    .then(function (response) {
								...
								$scope.viewanswers=function(forum)
								{
									...
									console.log("forumid:"+$scope.answerforum.forum_id);
									 $http.get("http://localhost:8081/Chatworld/viewAnswers/"+$scope.answerforum.forum_id)
									    .then(function (response) {
				...
				chatworld.controller("friendslistController",function($scope,$http,$rootScope)	
						{	
					$rootScope.userhome=true;
					$rootScope.userforum=true;
					$rootScope.userjobs=true;
					...
					console.log("name in  friendslist:"+$rootScope.uname);
							 $http.get("http://localhost:8081/Chatworld/viewFriends/"+$rootScope.uname)
							    .then(function (response) {
				...
				chatworld.controller("allblogsController",function($scope,$http,$rootScope,$route)	
						{	
					$rootScope.userhome=true;
					$rootScope.userforum=true;
					$rootScope.userjobs=true;
					...
					console.log("username in allblog controller:"+$rootScope.uname);
					 ...
					 $http.get("http://localhost:8081/Chatworld/viewComments/"+$scope.viewcomments.blog_id)
								    .then(function (response) {
						...
						$scope.comment=function(allblogs)
						{
							...
							$http.post('http://localhost:8081/Chatworld/addComment',comment);
							 $http.get("http://localhost:8081/Chatworld/viewBlogs")
							    .then(function (response) {
